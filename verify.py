...
def step_unpack(env: ISSEnvironment, action: Action):
    obs = env.step(action)
    reward_val = float(obs.reward or 0.0)
    done = bool(obs.done)
    reward_breakdown = (obs.metadata or {}).get("reward_components", {})
    reward_breakdown["score"] = reward_val
    info = (obs.metadata or {}).get("info", {})
    return obs, reward_val, done, reward_breakdown, info
...
def run_tests():
    ...
    obs, reward_score, done, reward_data, info = step_unpack(env, action1)
    assert not done, "Should not be done after inspect"
    assert reward_score == 0.0, f"Expected 0.0 (non-terminal), got {reward_score}"
    print(f"  ✓ inspect_object: done={done}, reward.score={reward_score}")
    ...
    obs, reward_score, done, reward_data, info = step_unpack(env, Action(
    ...
    print(f"  ✓ submit_report: done={done}, reward.score={reward_score:.3f}")
    print(f"    breakdown={reward_data}")
    assert reward_score > 0.8, f"Expected score > 0.8, got {reward_score:.3f}"
    print(f"  ✓ Score {reward_score:.3f} > 0.8")
    ...
    obs, reward_score, done, reward_data, info = step_unpack(env, dangerous_action)
    ...
    obs, reward_score, done, reward_data, info = step_unpack(env, pull_action)
    ...
    assert 0.0 <= reward_score <= 1.0, f"score {reward_score} out of range"
    ...
    obs3, reward3_score, done3, reward3_data, _ = step_unpack(env2, Action(action_type="identify_root_cause",
                                                                            target_object_id="FE_service_01",
                                                                            reasoning="Extinguisher discharged before alarm — propellant interfered with nitrogen sensor"))
    assert 0.0 <= reward3_score <= 1.0, f"Investigation score {reward3_score} out of range"
    print(f"  ✓ Investigation score={reward3_score:.3f} (in range)")
    print(f"    breakdown={reward3_data}")
